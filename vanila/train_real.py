...
from dataset import BenchmarkTest, RealH5
...
def train_model(net, datasets, optimizer, lr_scheduler, criterion):
    ...
    writer = SummaryWriter(args.log_dir + '_real')
    for epoch in range(args.epoch_start, args.epochs):
        ...
        if (epoch+1) % args.save_model_freq == 0 or epoch+1==args.epochs:
            model_prefix = 'model_'
            save_path_model = os.path.join(args.model_dir+'_real', model_prefix+str(epoch+1))
            torch.save({
                'epoch': epoch+1,
                'step': step+1,
            ...
            save_path_model_state = os.path.join(args.model_dir+'_real', model_state_prefix+str(epoch+1))
...
def main():
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, 100, 0.75)
    ...
    if args.resume:
        ...
        args.epoch_start = 0
        if os.path.isdir(args.log_dir+'_real'):
            shutil.rmtree(args.log_dir+'_real')
        os.makedirs(args.log_dir+'_real')
        if os.path.isdir(args.model_dir + '_real'):
            shutil.rmtree(args.model_dir+'_real')
        os.makedirs(args.model_dir+'_real')

    for arg in vars(args):
        print('{:<15s}: {:s}'.format(arg,  str(getattr(args, arg))))

    datasets = {'train':RealH5(gt_h5_path='/home/eunu/nas/DLA/real_gt.h5', noise_h5_path='/home/eunu/nas/DLA/real_noisy.h5'),
                                        'test_SIDD':BenchmarkTest('/home/eunu/nas/DLA/test_data/small_imgs_test.hdf5')}
    # train model
